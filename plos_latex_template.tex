...
As observed in Fig.\ref{fig:Fig4}A, there is a notable transformation in the plant height post-adjustment post PC residualization. The distribution compresses into a constrained, mean-zero residual with reduced variability, in contrast to the raw phenotype, which spans a wide, right-shifted spectrum. This observation suggests that a considerable portion of the variation in height is associated with the population structure captured by the PCs. Removing this component was important to model the lipid prediction to ensure that subsequent models do not erroneously interpret genetic-background differences as biological variation. Fig.\ref{fig:Fig4}B is an example of lipid species, TG(10:0/10:0/10:0). Post PC adjustment, there is only a modest shift in density characterized by a slight recentering and minor change in variance, while the overall shape remains unimodal and similar to the original distribution. It indicates that the population structure accounts for only a minimal portion of the variability. Thus, the majority of the signal is preserved post-adjustment. Collectively, the panels demonstrate that the PC residualization effectively removes strong structure from the phenotype while maintaining lipid abundances mostly intact, which is important for predictive modeling and TreeSHAP interpretation.

The random-forest regressor model demonstrated consistent generalization across five folds of CV on the training dataset (Fig. \ref{fig:Fig4}C). The mean performance across the folds is characterized by RMSE $=27.85$, MAE $=17.99$, and $R^2=0.680$ (dashed lines), with only minor variations between folds, suggesting that the accuracy is not reliant on any single partition. Evaluation on the test set (Fig.\,\ref{fig:Fig4}D) resulted in RMSE $=24.213$ and $R^2=0.688$. The predicted residual phenotypes align closely with the best fit line, showing slight compression at the extremes but lacking any apparent systematic bias. This outcome verifies that the combination of PC residualization and stratified splitting produced a model capable of generalizing beyond the training data.

To elucidate the fitted forest model, we computed TreeSHAP values for all lipid entities. Fig.\,\ref{fig:Fig4} panel~\textbf{E} illustrates the principal features, ordered by their global significance. Each data point corresponds to a sample; the $x$-axis represents the sample-specific SHAP value, reflecting its contribution to the prediction of the residual phenotype, while the color of the point denotes the standardized lipid abundance. Colors positioned on the positive side suggest that lipids with greater abundance are inclined to \emph{increase} the prediction, whereas colors on the negative side suggest that an increased abundance of those lipids tends to \emph{decrease} the prediction. Panel~\textbf{E} displays the related global importance as the mean absolute SHAP (mean,,$\phi$) per lipid, independent of directionality. For plant height, the hierarchy consists primarily of several short- to medium-chain TG (e.g., TG species enriched with 10:0–12:0 chains), supplemented by components from neutral and plastid membranes (e.g., MG/MGDG, selected PCs, and SQDG). The plots infer that increased abundance of significant TGs generally exerts a consistent influence on predictions, whereas numerous phospho/galactolipid attributes exhibit variable bidirectional effects across samples. 

\subsection*{Cross–study prediction and transferability of lipid features.} \\
To test whether our lipidomics to predict phenotypes observed in an independent study, we employed plant height from Lucas Boatwright’s sorghum experiment, conducted under analogous growth conditions in a comparable "control" context, as the response variable, with our lipid profiles serving as predictors. The model's performance on this external dataset exhibited metrics of \emph{RMSE} = 18.6, \emph{MAE} = 13.3, Pearson’s $r = 0.636$, $R^2 = 0.405$, and bias = $+5.41$ (with predictions exceeding the actual values by an average of 5.41 units), aligning closely with the accuracy observed in our field trials (Supplementary Table~4). TreeSHAP analysis revealed several top-ranked lipids common to both the external and our datasets, suggesting that the most informative lipid signals are biologically consistent across different fields. This concordance prompted us to apply the identical analytical pipeline to additional SAP phenotypes assessed under similar conditions—including stem diameter, \textbeta-carotene, lutein, zeaxanthin, grain number per primary panicle, thousand-grain weight, and grain yield per primary panicle—and we present per-phenotype metrics and prominent SHAP features in the Supplementary Table 5.


\subsubsection*{Salient TreeSHAP features across phenotypes.}


\subsubsection{Plant height: TreeSHAP interpretation}

\emph{Triacylglycerols}
The feature ranking is overwhelmingly enriched for medium–chain triacylglycerols (TGs), e.g.\ \textit{TG(10{:}0/10{:}0/10{:}0)}, \textit{TG(10{:}0/12{:}0/16{:}0)}, \textit{TG(12{:}0/12{:}0/14{:}0)}, \textit{TG(12{:}0/12{:}0/16{:}1)}, with many additional medium–chain TGs among the top hits. Medium–chain fatty acids are rapidly funneled through $\beta$-oxidation, providing a ``fast-burn'' carbon/ATP source. This pattern indicates that taller plants are those with larger, more accessible neutral-lipid reserves that can be mobilized to meet the steep carbon and energy demands of stem elongation (cell wall biosynthesis, lignification, and meristematic growth).
...
\emph{Hormones}
Gibberellic acid (0.053) appears among the top predictors, matching its well-known role in promoting flowering. The apocarotenoid α-ionone (0.051) hints at carotenoid breakdown/signaling intersecting with developmental timing—consistent with stress/redox cues that often accompany the switch.
...
\emph{Membrane remodeling and lipid precursors}
...
\subsubsection{Stem diameter:} Plastid membrane lipids ranked highest, led by SQDG(18{:}3/18{:}3), with DGDG(16{:}0/18{:}1) also prominent; growth‐related signals such as gibberellic acid and the oxylipin 9,12,13‐TriHODE appeared among the top features, alongside PA(16{:}0/18{:}2) and several medium/long‐chain TGs. This points to a link between plastid anionic/galactolipid remodeling, oxylipin signaling, and stem thickening.\\


\subsubsection{β‐Carotene:} \\
The top signals are not random. They cohere into four biochemical themes that make sense for a plastid-derived, fat-soluble pigment.

\emph{Neutral-lipid storage context}. Many of the strongest predictors are TGs for example TG(12:0/16:0/18:2), TG(14:0/18:3/18:3), TG(12:0/18:2/18:3), TG(12:0/18:1/18:1), TG(12:0/18:1/18:3), and TG(8:0/16:1/18:1). Because carotenoids partition into lipid droplets, a TG-rich, PUFA-bearing background likely provides both solubilization and sequestration capacity, stabilizing β-carotene at higher levels. In short, TG abundance/composition behaves like a storage-capacity proxy for carotenoid load.
...
\emph{Direct pathway sentinels and stress/redox markers}. On-pathway molecules like apocarotenal (a carotenoid breakdown product), β-carotene itself, and α-carotene appear among the top features, providing an internal validity check that the model is capturing carotenoid metabolism. Ubidecarenone (CoQ10)—another isoprenoid—implicates shared precursor and redox state across isoprenoid branches. Oxylipin trans-EKODE and γ-linolenoyl ethanolamide (NAE 18:3) indicate stress and redox signaling commonly co-regulated with antioxidant carotenoids. Finally, DG(16:0/2:0), DG(18:1/2:0), and MG(20:4) sit at key lipid branch points and act as second messengers, reflecting the remodeling that accompanies high carotenoid accumulation.


\subsubsection{Lutein:} 
It’s consistent with carotenoid biochemistry and with what you saw for β-carotene, but tuned to xanthophyll biology.

\emph{Carotenoid network}
The model puts lutein itself at the top , with strong support from zeaxanthin , $\alpha$-carotene , and $\beta$-cryptoxanthin. That quartet is exactly what you’d expect if the model is reading out the carotenoid biosynthetic and functional neighborhood: $\alpha$-carotene feeds the $\varepsilon$,$\beta$ branch that yields lutein; zeaxanthin is a partner xanthophyll in photoprotection (xanthophyll cycle); $\beta$-cryptoxanthin sits in the $\beta$-branch and often co-varies under high-light/redox cues. In short, the model isn’t latching onto a single compound—it’s capturing the co-regulated carotenoid module that travels with high lutein.
...
\subsubsection{Zeaxanthin}

We modeled zeaxanthin from lipidomics with a random forest and interpreted feature effects using mean absolute SHAP values ($\overline{|\phi|}$). Larger $\overline{|\phi|}$ indicates a stronger average contribution to the prediction across genotypes.

\emph{The xanthophyll cycle}
A defining result is the prominence of the xanthophyll cycle intermediate \textit{antheraxanthin}, indicating that zeaxanthin variation is driven by enzyme-level conversion dynamics (VDE/ZE) rather than only steady-state pools. Genotypes with higher antheraxanthin{\,$\leftrightarrow$\,}zeaxanthin flux under stress/light show the highest zeaxanthin.
...
Zeaxanthin is predicted by a four-part system: (i) \emph{xanthophyll-cycle activity} (antheraxanthin), (ii) a \emph{hormonal axis} (GA) coordinating development and photoprotection, (iii) \emph{thylakoid composition} (DGDG/SQDG) setting the functional membrane context, and (iv) \emph{neutral-lipid capacity} (PUFA-rich TGs) for sequestration. Compared with lutein’s more constitutive network profile, zeaxanthin reads out as a conversion-driven, stress-responsive phenotype.


\subsubsection{Grain number per primary panicle:}

The #1 feature, MG(20:4) (MeanAbs = 7.08), is a lipid-turnover flag: monoacylglycerols rise when lipases remodel membranes. Together with high-ranking DG(16:0/18:0) and DG(16:1/18:3), plus head-group lipids (PC(14:0/18:2), PC(16:1/16:1), PS(18:0/18:2), PE(18:0/18:2)), the profile points to intense membrane recycling and resynthesis—exactly what’s needed during floret initiation, meiosis, and rapid cell division in the developing panicle.
...
What TGW is capturing. In the RF + TreeSHAP readout, TGW looks like a build-and-protect phenotype: seeds that can (i) assemble membranes and oil bodies efficiently and (ii) protect unsaturated stores from oxidative damage end up heavier.
