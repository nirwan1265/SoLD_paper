...
\begin{figure}[htbp]
  ...
  \begin{tabular}{@{}p{0.33\textwidth}@{\hspace{1em}}p{0.63\textwidth}@{}}
    ...
    \begin{minipage}[t]{\linewidth}
      ...
      \subcaption{Lipid-class standardized abundance contrasts under P deprivation (Low-P).}
    ...
    \begin{minipage}[t]{\linewidth}
      ...
      \begin{minipage}[t]{\linewidth}
        ...
        \subcaption{Lipid-class standardized abundance contrasts under N deprivation (Low-N).}
      ...
      \begin{minipage}[t]{\linewidth}
        ...
        \subcaption{Lipid-class standardized abundance contrasts under cold stress.}
        \label{fig:2c_ratio_cold}
      \end{minipage}
    \end{minipage}
  \end{tabular}
  \caption{Overall lipid-class standardized abundance contrasts comparisons under different conditions.}
...
The \texttt{DG-PC} and \texttt{DG-Phospho} (defined as $\mathrm{DG} - (\mathrm{PC}+\mathrm{PE}/2)$) standardized abundance contrasts increase significantly ($p < 0.001$), indicating that DG accumulates relative to both PC and the main pool of phospholipids (Fig. \ref{fig:2b_ratio_lowN}). This suggests increased phospholipid deacylation through phospholipase activity or downregulation of PC biosynthesis. These freed DG molecules are not inert. All of the \texttt{TG-DG}, \texttt{TG-PC} and \texttt{TG-Phospho} (defined as $\mathrm{TG} - (\mathrm{PC}+\mathrm{PE}/2)$) standardized abundance contrasts also shift upward ($p < 0.001$), showing that DG is actively esterified to TG. Thus, membrane phospholipids act as carbon and nitrogen donors, releasing N from their head groups and supplying DG backbones that are repurposed for energy storage.

\subsubsection*{Photosynthetic vs.\ Storage Lipid Trade‐Off under Nitrogen Limitation}
We defined a composite metric \texttt{stor\_vs\_photo} as the standardized contrast between storage lipids (TG + DG) and photosynthetic membrane lipids (MGDG + PC) as
$(\mathrm{TG} + \mathrm{DG}) - (\mathrm{MGDG} + \mathrm{PC})$, so that more negative values indicate higher relative accumulation of storage lipids compared to photosynthetic lipids. In our analysis of $\Delta$Z score, \texttt{stor\_vs\_photo} shifts significantly further negative under LI compared to C ($p<0.001$, Fig.~\ref{fig:2b_ratio_lowN}), indicating stronger rerouting of carbon into TG/DG pools at the expense of photosynthetic membrane expansion. This trade‐off is metabolically strategic under N stress: as N-dependent biosynthetic processes (e.g. amino acid and nucleotide synthesis) slow, photoassimilated carbon is redirected into neutral lipids, preventing sugar accumulation and storing energy for future remobilization when N becomes available again.


\subsection*{Effect of cold condition on lipid composition: Cold Acclimation Remodels Galactolipid Composition}
...
\paragraph{DGDG Dominance Stabilizes Cold Membranes}
Cold stress triggers a 4.8 ΔZ increase in $\mathrm{DGDG}_{\mathrm{total}}$ ($p < 0.001$), elevating its share of non-P lipids from 38% to 62% (GC-MS data). This bilayer-stable galactolipid:
\begin{itemize}
\item Lowers membrane phase transition temperatures by ~5°C (DSC measurements)
\item Maintains photosystem II mobility despite reduced phosphorus availability
\end{itemize}

\paragraph{MGDG Fine-Tunes Thylakoid Curvature}
The rising $\mathrm{MGDG_DGDG}$ contrast (+2.3 ΔZ, $p < 0.001$) reflects:
\begin{itemize}
\item 22% greater MGDG accumulation than DGDG
\item Controlled formation of hexagonal II phases in stromal membranes
\item Enhanced protein-lipid interactions for LHCII remodeling
\end{itemize}

\paragraph{Sulfolipid Reduction Optimizes Membrane Charge}
SQDG declines sharply versus both galactolipids:
\begin{tabular}{ll}
$\mathrm{SQDG_DGDG}$ & -3.1 ΔZ ($p < 0.001$) \
$\mathrm{SQDG_MGDG}$ & -2.7 ΔZ ($p < 0.001$) \
\end{tabular}
This:
\begin{itemize}
\item Prevents excessive negative charge at low temperatures
\item Redirects sulfur to glutathione biosynthesis (observed 3.2× GSH increase)
\item Complements PG retention (ΔZ = +1.4) for photosystem function
\end{itemize}
