...
(\textbf{C}) The training set underwent a five-fold cross-validation. Points represent fold-wise Root Mean Square Error (RMSE), Mean Absolute Error (MAE), and $R^2$; dashed lines indicate the mean across folds, suggesting consistent performance across different splits. (\textbf{D}) An agreement plot on the \emph{held-out} test set illustrates the Random Forest (RF)-predicted residual phenotype in comparison with the observed residual phenotype; the dashed line represents the identity ($y=x$). The annotated RMSE and $R^2$ provide a summary of out-of-sample error and explained variance. The dispersion of the cloud around the identity line illustrates calibration, potential bias, and outliers.

To interpret the fitted forest, we calculated precise TreeSHAP values for all lipids. Panel~\textbf{C} presents a SHAP \\"beeswarm\\" plot for the leading features, ordered by global importance. Each point represents a line; the $x$-axis denotes the sample-specific SHAP value, indicating its contribution to the prediction of residual flowering time, while the point's color reflects the standardized lipid abundance (cool colors representing low abundance and warm colors, high abundance). Warm colors located on the positive side suggest lipids whose greater abundance tends to \emph{increase} the prediction, whereas cool colors on the negative side suggest lipids whose increased abundance tends to \emph{decrease} the prediction. Panel~\textbf{D} displays the associated global importance as the mean absolute SHAP (mean\\,$\phi$) per lipid, offering an order-of-magnitude ranking that is direction-independent. The ranking is principally comprised of several short/medium-chain triacylglycerols (e.g., TG species enriched with 10:0–12:0 chains), supplemented by contributors from neutral and plastid membranes (e.g., MG/MGDG, selected PCs, and SQDG). The beeswarm plots indicate that higher abundance of prominent TGs generally influences predictions consistently, whereas several phospho/galactolipid features show variable bidirectional effects across lines. In conjunction with the shifts in class/subclass composition (increase in TG and DG; decrease in MGDG/SQDG; PC$\downarrow$/PE$\uparrow$) and the network/GWAS evidence (DGATs, PNPLA1, SQD2, PLD, LCAT-like), the SHAP analysis identifies neutral-lipid remodeling as the predominant predictor of the phenotype, while concurrently acknowledging plastid and phospholipid remodeling signals."
}
