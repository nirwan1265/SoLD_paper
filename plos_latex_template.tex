...
% noindent \textbf{Network-level shifts (LINEX2).}
...
%========================================================
% Supplementary Figure S6 - OPLS-DA permutation tests
%========================================================
\begin{figure}[htp]
  \centering
  \includegraphics[width=\textwidth]{fig/supp/SuppFig6.png}
  \caption{
    Permutation validation of the OPLS--DA model comparing Control (C) vs.\ LowInput (LI).
    {\bf(A)} Distribution of permuted $R^2_Y$ values (500 label permutations). 
    The observed $R^2_Y = 0.979$ (arrow) lies far outside the null distribution ($p = 0.002$). 
    {\bf(B)} Distribution of permuted $Q^2$ values. The observed $Q^2 = 0.978$ (arrow) is likewise extreme relative to permutations ($p = 0.005$). 
    Both results confirm that the model’s explanatory and predictive power significantly exceed chance expectations.
  }
  \label{fig:S6}
\end{figure}
